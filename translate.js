...
try {
  let html = fs.readFileSync('C:\\\\Users\\\\josed\\\\Proyectos\\\\ingenylabs\\\\index.html', 'utf-8');
  ...
  const translations = {
    ...
    '<a href=\"#nosotros\" class=\"nav-link hover:text-ingeny-green transition-colors\">Nosotros</a>': '<a href=\"#nosotros\" class=\"nav-link hover:text-ingeny-green transition-colors\">Us</a>',
    '<a href=\"#servicios\" class=\"nav-link hover:text-ingeny-green transition-colors\">Servicios</a>': '<a href=\"#servicios\" class=\"nav-link hover:text-ingeny-green transition-colors\">Services</a>',
    '<a href=\"#casos-uso\" class=\"nav-link hover:text-ingeny-green transition-colors\">Casos de Uso</a>': '<a href=\"#casos-uso\" class=\"nav-link hover:text-ingeny-green transition-colors\">Use Cases</a>',
    '<a href=\"#metodologia\" class=\"nav-link hover:text-ingeny-green transition-colors\">Metodología</a>': '<a href=\"#metodologia\" class=\"nav-link hover:text-ingeny-green transition-colors\">Methodology</a>',
    '<a href=\"#contacto\" class=\"nav-link hover:text-ingeny-green transition-colors\">Contacto</a>': '<a href=\"#contacto\" class=\"nav-link hover:text-ingeny-green transition-colors\">Contact</a>',
    '<a href=\"#nosotros\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Nosotros</a>': '<a href=\"#nosotros\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Us</a>',
    '<a href=\"#servicios\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Servicios</a>': '<a href=\"#servicios\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Services</a>',
    '<a href=\"#casos-uso\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Casos de Uso</a>': '<a href=\"#casos-uso\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Use Cases</a>',
    '<a href=\"#metodologia\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Metodología</a>': '<a href=\"#metodologia\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Methodology</a>',
    '<a href=\"#contacto\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Contacto</a>': '<a href=\"#contacto\" class=\"mobile-link hover:text-ingeny-green transition-colors\">Contact</a>',
    'Agenda una auditoría': 'Schedule an audit'
  };

  for (const [es, en] of Object.entries(translations)) {
    // Escape string to create a regex that avoids failing on characters like ? or (
    const escapedEs = es.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&');
    html = html.replace(new RegExp(escapedEs, \"g\"), en);
  }

  // Language switcher for en.html
  const switcherEN = \`<div class=\"flex gap-2 text-sm font-bold bg-white/5 rounded-full p-1 border border-white/10\">
                <a href=\"index.html\" class=\"px-3 py-1 text-white hover:text-ingeny-green transition-colors rounded-full\">ES</a>
                <a href=\"en.html\" class=\"px-3 py-1 bg-ingeny-green text-black rounded-full\">EN</a>
            </div>\`;

  // We add the language switcher before the wa.me links
  // But there are multiple wa.me links. We only want it in the header, which is around line 92:
  // <div class=\"hidden md:flex items-center gap-6\">
  //     <a href=\"https://wa.me/56954679270...
  html = html.replace(/<div class=\"hidden md:flex items-center gap-6\">\\s*<a href=\"https:\\/\\/wa\\.me/g, '<div class=\"hidden md:flex items-center gap-6\">\\n            ' + switcherEN + '\\n            <a href=\"https://wa.me');

  // Same thing for mobile-menu, we will add the language switcher below the close-button?
  // Actually we should add it in the mobile menu container.
  const mobileSwitcherEN = \`<div class=\"flex gap-2 text-xl font-bold bg-white/5 rounded-full p-1 border border-white/10 mb-4\">
            <a href=\"index.html\" class=\"px-4 py-2 text-white hover:text-ingeny-green transition-colors rounded-full\">ES</a>
            <a href=\"en.html\" class=\"px-4 py-2 bg-ingeny-green text-black rounded-full\">EN</a>
        </div>\`;
  html = html.replace(/<a href=\"#nosotros\" class=\"mobile-link/g, mobileSwitcherEN + '\\n        <a href=\"#nosotros\" class=\"mobile-link');

  fs.writeFileSync('C:\\\\Users\\\\josed\\\\Proyectos\\\\ingenylabs\\\\en.html', html);
  console.log('Translated successfully to en.html');
} catch (e) {
  console.error(e);
}
